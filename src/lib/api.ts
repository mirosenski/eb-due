  ...
  stations: {
    ...
    getAll: async (): Promise<PoliceStation[]> => {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          console.error('Fehler beim Laden der Stationen:', errorData);
          throw new ApiError(
            errorData.error || `HTTP ${response.status}`,
            response.status,
            errorData
    ...
    create: async (data: CreateStationData): Promise<PoliceStation> => {
      try {
        console.log('Sende POST-Request an:', `${API_BASE}/stationen`);
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          console.error('Fehler beim Erstellen der Station:', errorData);
          throw new ApiError(
            errorData.error || `HTTP ${response.status}`,
            response.status,
            errorData
        ...
        console.log('Station erfolgreich erstellt:', newStation);
        return newStation;
      } catch (error) {
