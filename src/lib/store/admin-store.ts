...
export const useAdminStore = create<AdminState>((set, get) => ({
  ...
  updateStation: async (id, updates) => {
    ...
    try {
      // Rufe die echte API auf
      const { updateStation: apiUpdateStation } = await import('@/services/api/backend-api.service');
      await apiUpdateStation(id, updates);
  ...
  deleteStation: async (id) => {
    ...
    try {
      // Rufe die echte API auf
      const { deleteStation: apiDeleteStation } = await import('@/services/api/backend-api.service');
  ...
  getFilteredStations: () => {
    ...
    // Sicherheitscheck: Stelle sicher, dass allStations ein Array ist
    if (!Array.isArray(allStations)) {
      console.warn('allStations ist kein Array:', allStations);
      set({ filteredStations: [] });
      return [];
    }
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(station => {
        // Sicherheitscheck für station und seine Eigenschaften
    ...
    if (cityFilter) {
      filtered = filtered.filter(station => {
        if (!station || typeof station !== 'object') return false;
        return station.city === cityFilter;
      });
    }
    ...
    if (typeFilter !== 'all') {
      filtered = filtered.filter(station => {
        if (!station || typeof station !== 'object') return false;
        return station.type === typeFilter;
      });
    }
