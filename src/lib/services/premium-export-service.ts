...
class PremiumExportService {
  ...
  async exportToPremiumExcel(
    ...
    try {
      ...
      const fileName = `eb-due_Premium_Export_${timestamp}.xlsx`;
  ...
  private createMetaData(routeResults: RouteResult[], startAddress: Address): MetaData {
    ...
    return {
      ...
      exportiert_von: 'eb-due v2.0 - Dienststellen und Einrichtungen Baden-Württemberg',
  ...
  private createPoliceHeaderSheet(): XLSX.WorkSheet {
    const headerData = [
      ['eb-due v2.0 - Dienststellen und Einrichtungen Baden-Württemberg'],
      ['Professionelle Routing-Anwendung für Polizeieinsätze'],
      [''],
      ['SYSTEM-INFORMATIONEN'],
      [''],
      ['Version', 'eb-due v2.0'],
      ['Entwickelt für', 'Dienststellen und Einrichtungen Baden-Württemberg'],
      ...
      ['KONTAKT & SUPPORT'],
      [''],
      ['E-Mail Support', 'support@eb-due.de'],
      ['Telefon', '+49 (0) 711 999-1234'],
      ['Website', 'https://ppstuttgart.polizei-bw.de'],
      ['Dokumentation', 'https://docs.eb-due.de'],
      [''],
      ['© 2025 Dienststellen und Einrichtungen Baden-Württemberg - Alle Rechte vorbehalten']
    ];
  ...
  async exportToCSV(
    ...
    try {
      ...
      link.download = `eb-due_Export_${new Date().toISOString().split('T')[0]}.csv`;
      link.click();
  ...
  async exportToPremiumPDF(
    ...
    try {
      ...
      pdf.text('eb-due v2.0', pageWidth / 2, 12, { align: 'center' });
      
      pdf.setFontSize(14);
      pdf.text('Dienststellen und Einrichtungen Baden-Württemberg', pageWidth / 2, 20, { align: 'center' });
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        pdf.text(
          `Seite ${i} von ${totalPages} - eb-due v2.0 © Dienststellen und Einrichtungen Baden-Württemberg`,
          pageWidth / 2,
          pageHeight - 10,
          { align: 'center' }
        );
      ...
      // Save PDF
      const timestamp = new Date().toISOString().split('T')[0];
      const fileName = `eb-due_Premium_Report_${timestamp}.pdf`;
      pdf.save(fileName);
