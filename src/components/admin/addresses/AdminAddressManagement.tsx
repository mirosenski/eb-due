...
const AdminAddressManagement: React.FC = () => {
  ...
  const allCities = useMemo(() => {
    const cities = new Set(Array.isArray(addresses) ? addresses.map(addr => addr.city) : [])
  ...
  const handleCheckAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(Array.isArray(filteredAddresses) ? filteredAddresses.map(addr => addr.id) : [])
    } else {
      setSelectedIds([])
    }
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
        ...
        {addresses.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
            {Array.isArray(filteredAddresses) && filteredAddresses.map((address) => (
