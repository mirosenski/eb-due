...
export const cleanDatabase = async () => {
  ...
  try {
    ...
    if (deleteStationsResponse.ok) {
      const stationResult = await deleteStationsResponse.json().catch(() => ({ deletedCount: 0 }));
      console.log('✅ Alle Stationen gelöscht:', stationResult.deletedCount);
    } else {
      console.warn('⚠️ Fehler beim Löschen der Stationen:', deleteStationsResponse.status);
      const errorText = await deleteStationsResponse.text().catch(() => 'Unbekannter Fehler');
      console.warn('Fehlerdetails:', errorText);
    }
    ...
    if (deleteAddressesResponse.ok) {
      const addressResult = await deleteAddressesResponse.json().catch(() => ({ deletedCount: 0 }));
      console.log('✅ Alle Adressen gelöscht:', addressResult.deletedCount);
    } else {
      console.warn('⚠️ Fehler beim Löschen der Adressen:', deleteAddressesResponse.status);
      const errorText = await deleteAddressesResponse.text().catch(() => 'Unbekannter Fehler');
      console.warn('Fehlerdetails:', errorText);
    }
