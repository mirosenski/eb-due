// eb-due Maps Service Worker
// Handles offline caching for map tiles and routing data

const CACHE_VERSION = 'v1.2.0';
const CACHE_NAME = 'eb-due-maps-v1';
const RUNTIME_CACHE = 'eb-due-maps-runtime';
    ...
    caches.keys()
      .then(cacheNames => {
        return Promise.all(
          cacheNames
            .filter(cacheName => 
              cacheName.startsWith('eb-due-maps-') && 
...
self.addEventListener('push', event => {
  if (event.data) {
    ...
    if (data.type === 'offline-package-ready') {
      event.waitUntil(
        self.registration.showNotification('eb-due', {
...
console.log('eb-due Maps Service Worker loaded');
